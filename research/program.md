...
1. **Agree on a run tag**: propose a tag based on today's date (e.g. `apr6b`). The branch `autoresearch/<tag>` must not already exist — this is a fresh run.
...
- **CLIP text embeddings for discriminator conditioning**: The discriminator receives precomputed CLIP embeddings (dim=512) directly via `--clip_embeddings_dir`. This is now required (no fallback to hidden-state text pooling).
...
**Best KID = 0.058232** (-15.5% vs untrained 0.068920)
...
1. **Noise schedule is the most impactful knob.** RENOISE_M=1.0 (sigmoid ~0.73, biased toward high noise) gave the single biggest improvement: -12% KID vs M=0.5. At M=1.0, real and fake samples look more similar under noise, giving the student a gentler learning signal. M=1.5 (sigmoid ~0.82) was too much noise — the discriminator couldn't distinguish real from fake (logit_gap dropped to 1.3).

2. **GI=3 is the optimal update interval with CLIP conditioning.** Tested GI={2,3,4,5}. GI=2 gave the disc too few steps relative to the generator (KID worse than untrained). GI=4 and GI=5 weakened the learning signal. GI=3 was consistently best across both M=0.5 and M=1.0.

3. **Default learning rates are optimal.** STUDENT_LR=5e-6 and DISC_LR=1e-5 (2:1 ratio) won. Higher student LR (1e-5) caused severe overshoot (+38% worse). Higher disc LR (5e-5) also hurt. The learning rates found in the pre-CLIP search still hold.

4. **Discriminator architecture should not be weakened.** Both fewer layers ([10,20,29] vs [5,10,15,20,25,29]) and smaller hidden dim (128 vs 256) produced worse results. The disc needs broad coverage across transformer depth and sufficient capacity.

5. **Warmup doesn't help at 500 steps.** LR_WARMUP=50 was second-best overall but still 10% worse than no warmup. With only 500 steps, warmup wastes 10% of the training budget.

6. **logit_gap as a health signal.** Best runs had logit_gap ~2-3. Runs with logit_gap < 1.5 (M=1.5, GI=4, disc_hidden=128) consistently underperformed — the disc wasn't providing enough learning signal.

### What to explore next

- **Longer training** (2000-5000 steps) with the best config to see if improvements compound
- **10K training data** — current search used 3K subset; more data may shift optimal hyperparameters
- **RENOISE_M fine-tuning** around 1.0 (try 0.75, 1.25) — we know 0.5 and 1.5 are worse but haven't explored the neighborhood
- **GI=2 with lower disc LR** — GI=2 was too aggressive at dlr=1e-5 but might work at dlr=5e-6
