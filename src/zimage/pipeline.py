...
def generate(
    ...
    if num_images_per_prompt > 1:
        ...
        if do_classifier_free_guidance:
            negative_prompt_embeds_list = [npe for npe in negative_prompt_embeds_list for _ in range(num_images_per_prompt)]
    ...
    for i, t in enumerate(tqdm(timesteps, desc="Denoising", total=len(timesteps))):
        ...
        if apply_cfg:
            latents_typed = latents.to(transformer.dtype if hasattr(transformer, 'dtype') else next(transformer.parameters()).dtype)
