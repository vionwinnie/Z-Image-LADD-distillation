...
# Add VideoX-Fun to path for baseline training imports
_videox_fun_root = os.environ.get("VIDEOX_FUN_ROOT", os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(current_file_path))), "VideoX-Fun"))
if os.path.isdir(_videox_fun_root):
    sys.path.insert(0, _videox_fun_root) if _videox_fun_root not in sys.path else None

# Add src/ for LADD imports
_src_root = os.path.join(os.path.dirname(os.path.dirname(current_file_path)), "src")
sys.path.insert(0, _src_root) if _src_root not in sys.path else None
...
def parse_args():
    ...
    # ---- LADD distillation flags ----
    parser.add_argument("--use_ladd", action="store_true",
                        help="Enable LADD distillation mode instead of baseline MSE training.")
    parser.add_argument("--learning_rate_disc", type=float, default=1e-4,
                        help="[LADD] Learning rate for discriminator heads.")
    parser.add_argument("--gen_update_interval", type=int, default=5,
                        help="[LADD] Update student every N discriminator steps.")
    parser.add_argument("--disc_layer_indices", nargs="+", type=int,
                        default=[5, 10, 15, 20, 25, 29],
                        help="[LADD] Teacher layer indices for discriminator heads.")
    parser.add_argument("--disc_hidden_dim", type=int, default=256,
                        help="[LADD] Discriminator hidden dimension.")
    parser.add_argument("--disc_cond_dim", type=int, default=256,
                        help="[LADD] Discriminator conditioning dimension.")
    parser.add_argument("--student_timesteps", nargs="+", type=float,
                        default=[1.0, 0.75, 0.5, 0.25],
                        help="[LADD] Discrete timestep set for student denoising.")
    parser.add_argument("--warmup_schedule_steps", type=int, default=500,
                        help="[LADD] Steps before switching to main timestep schedule.")
    parser.add_argument("--renoise_m", type=float, default=1.0,
                        help="[LADD] Mean for logit-normal re-noising distribution.")
    parser.add_argument("--renoise_s", type=float, default=1.0,
                        help="[LADD] Std for logit-normal re-noising distribution.")
    parser.add_argument("--text_drop_ratio", type=float, default=0.1,
                        help="[LADD] Probability of dropping text for CFG training.")
    parser.add_argument("--num_inference_steps", type=int, default=4,
                        help="[LADD] Number of steps for validation sampling.")
    parser.add_argument("--wandb_run_name", type=str, default=None,
                        help="[LADD] W&B run name.")
    parser.add_argument("--wandb_entity", type=str, default="yeun-yeungs",
                        help="[LADD] W&B entity (team or username).")
...
# ---------------------------------------------------------------------------
# LADD-specific helpers (only used when --use_ladd is set)
# ---------------------------------------------------------------------------

def _ladd_imports():
    """Lazy import LADD components to avoid import errors when not using LADD."""
    from training.ladd_discriminator import LADDDiscriminator
    from training.ladd_utils import (
        TextDataset, add_noise, logit_normal_sample,
        encode_prompt as ladd_encode_prompt,
    )
    return LADDDiscriminator, TextDataset, add_noise, logit_normal_sample, ladd_encode_prompt


def sample_student_timestep(batch_size, global_step, student_timesteps, warmup_steps=500,
                            device="cpu", generator=None):
    """Sample student denoising timestep with warm-up schedule."""
...
def main_ladd(args):
    """LADD distillation training: adversarial training with teacher as discriminator backbone."""
    LADDDiscriminator, TextDataset, add_noise, logit_normal_sample, ladd_encode_prompt = _ladd_imports()
    ...
    # Load models
    # -----------------------------------------------------------------------
    logger.info("Loading models for LADD training...")

    # Student transformer (trainable) — use the same loading as baseline
    transformer3d = ZImageTransformer2DModel.from_pretrained(
        args.pretrained_model_name_or_path,
        subfolder="transformer",
        torch_dtype=weight_dtype,
    ...
    if args.transformer_path is not None:
        ...
        transformer3d.load_state_dict(state_dict, strict=False)
        del state_dict
        logger.info(f"Loaded student weights from {args.transformer_path}")

    student = transformer3d
    student.train()
    ...
    discriminator = LADDDiscriminator(
        feature_dim=student.config.get("dim", 3840),
    ...
    if accelerator.is_main_process:
        ...
        init_kwargs = {}
        if args.report_to in ("wandb", "all") and is_wandb_available():
            init_kwargs["wandb"] = {
    ...
    vae_scale_factor = 2 ** (len(vae.config.get("block_out_channels", [64])) - 1)
    ...
    in_channels = student.config.get("in_channels", 16)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with torch.cuda.amp.autocast(dtype=weight_dtype):
                ...
                # 3. Generate noise as student input
                latent_shape = (bsz, in_channels, height_latent, width_latent)
                noise = torch.randn(latent_shape, device=accelerator.device,
                ...
                if is_gen_step:
                    student_out, _ = accelerator.unwrap_model(student)(
                        student_input_list, student_t, prompt_embeds,
                ...
                real_noisy = add_noise(real_noise.float(), real_noise_2.float(), t_hat.float()).to(weight_dtype)
                ...
                # ---- Student update (every gen_update_interval steps) ----
                if is_gen_step:
                    # Teacher forward WITHOUT torch.no_grad() so gradients flow
                    # through the teacher's ops back to student_pred → student weights.
                    # Teacher weights are frozen (requires_grad=False) but the
                    # computation graph is preserved for backprop.
                    ...
                    _, fake_extras_grad = teacher(
                        fake_input_grad, t_hat, prompt_embeds, return_hidden_states=True,
                    )
            ...
            # Validation
            if args.validation_prompts is not None and global_step % args.validation_steps == 0:
                log_validation(vae, text_encoder, tokenizer, student, args, accelerator, weight_dtype, global_step)
    ...
    if accelerator.is_main_process:
        ...
        unwrapped_student = accelerator.unwrap_model(student)
        torch.save(unwrapped_student.state_dict(), os.path.join(student_save_dir, "pytorch_model.bin"))
        logger.info(f"LADD training complete. Final checkpoint at step {global_step}")
